...
class RereadingAnalysis:
    ...
    def students_using_relevant_words_by_context_and_question(self):
        """
        Return a list of tuples of the form (context, question, count), where count is
        the number of students who used relevant words in that context and question.
        ...
        flattened_data = RereadingAnalysis.transform_nested_dict_to_list(context_question_count_map)
        return flattened_data

    @staticmethod
    def transform_nested_dict_to_list(context_question_map):
        """
        Transforms a nested dictionary data structure into a flat array of tuples in the form
        (key1, key2, value).
        :param context_question_map: the map generated by
        students_using_relevant_words_by_context_and_question
        :return a list of tuples in the form (context, question, data)
        """
        flattened_list = []
        for context, questions in context_question_map:
            for question, data in context_question_map:
                flattened_list.append((context, question, data))
        return flattened_list

    def percent_using_relevant_words_by_context_and_question(self):
        """
        Return a list of tuples of the form (context, question, percent), where percent is
        ...
        context_question_count_list = self.students_using_relevant_words_by_context_and_question()

        context_question_percent_list = list(map(lambda count: count / student_count,
                                                 context_question_count_list))
        return context_question_percent_list
